...
public class UuidUtil {
	...
	public static String getInstallationId(Context context) {
		if (UuidUtil.uuid == null) {
			SharedPreferences prefs = async(context.getSharedPreferences(
					UuidUtil.FUNF_UTILS_PREFS, Context.MODE_PRIVATE));
			UuidUtil.uuid = prefs.getString(UuidUtil.INSTALLATION_UUID_KEY,
					null);
			if (UuidUtil.uuid == null) {
				UuidUtil.uuid = UUID.randomUUID().toString();
				prefs.edit()
						.putString(UuidUtil.INSTALLATION_UUID_KEY,
								UuidUtil.uuid).commit();
			}
		}
